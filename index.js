...
function getDateDetails(date) {
    ...
    const myDate = date;
    if (typeof myDate.getTime === "function") {
        return {
...
function setDateDetails(date, isoString) {
    ...
    const myDate = date;
    if (typeof myDate.getTime === "function") {
        const myIsoDate = new Date(isoString);
        if (typeof myIsoDate.getDate() === "number") {
            return myIsoDate;
        } else {
...
function nextNewYear() {
    ...
    const nextYearDate = new Date(Date.UTC(presentYear + 1, 0, 1));
    const diff = nextYearDate - nowDate;
    return `До нового року залишилось: Днів ${Math.floor(
        diff / (1000 * 60 * 60 * 24)
    )}, Годин ${Math.floor(
        (diff / (1000 * 60 * 60)) % 24
    )}, Хвилин ${Math.floor((diff / (1000 * 60)) % 60)}, Секунд ${Math.floor(
        (diff / 1000) % 60
...
function isLeapYear(year) {
    ...
    const myYear = year;
    if (typeof myYear === "number") {
        if (myYear % 4 == 0) {
            return { ["year"]: myYear, ["isLeap"]: true };
        } else {
            return { ["year"]: myYear, ["isLeap"]: false };
        }
    } else {
        return "Параметр має бути числом";
    }
}
...
function addDays(date, days) {
    ...
    if (typeof inputDate.getTime === "function") {
        const myDays = days;
        if (typeof myDays === "number") {
            const resultDate = inputDate;
            resultDate.setDate(resultDate.getDate() + myDays);
            return {
                ["inputDate"]: inputDate,
                ["addedDays"]: myDays,
...
function getDayOfWeek(date) {
    ...
    const inputDate = date;
    if (typeof inputDate.getTime === "function") {
        const dayNum = inputDate.getDay();
        return { ["inputDate"]: inputDate, ["dayOfWeek"]: daysOfWeek[dayNum] };
    } else {
        return "Помилка: вхідне значення має бути об'єктом Date";
    }
}
...
function getAge(birthDate) {
    ...
    if (typeof myDate.getTime === "function") {
        ...
        if (nowDate.getMonth() < myDate.getMonth()) {
            age--;
        } else if (nowDate.getMonth() == myDate.getMonth()) {
        ...
        console.log(age);
        return { ["birthDate"]: myDate, ["age"]: age };
    } else {
        return "Помилка: вхідне значення має бути об'єктом Date";
    }
}
...
function compareDates(date1, date2) {
    ...
    let compare;
    if (
        typeof myDate1.getTime === "function" &&
        typeof myDate2.getTime === "function"
    ) {
        if (myDate1 < myDate2) {
            compare = -1;
...
function getDaysDifference(startDate, endDate) {
    ...
    if (
        typeof myStartDate.getTime === "function" &&
        typeof myEndDate.getTime === "function"
    ) {
